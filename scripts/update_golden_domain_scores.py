...
Strategy:
    ...
    2. Apply the production sharder's shard_key() so each host collapses to the
       ...
       full host for entries in shard_split.yaml).
...
def collapse_to_domain_keys(
    ...
    can collapse to the same key when shard_split.yaml does not list them."""
    out: dict[str, set[int]] = {}
...
def main():
    ...
    try:
        ...
        _overrides, split_subdomains = load_sharding_config(INGEST_CONFIG, SPLIT_CONFIG)
